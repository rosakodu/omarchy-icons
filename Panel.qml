...
Panel {
    ...
    function applyTheme(themeName) {
        ...
        var script = 'THEME="$0"; '
            ...
            + '    find "$src" ! -type d \( -name "*.svg" -o -name "*.png" \) \( -path "*/apps/$sz/*" -o -path "*/applications/$sz/*" \) -exec cp -asf -t "$NEW_DIR/apps/" {} + 2>/dev/null || true; '
            + '  done; '
            + '  find "$src" ! -type d \( -name "*.svg" -o -name "*.png" \) \( -path "*/apps/scalable/*" -o -path "*/applications/scalable/*" -o -path "*/apps/*" -o -path "*/applications/*" \) -exec cp -asf -t "$NEW_DIR/apps/" {} + 2>/dev/null || true; '
            + '}; '
