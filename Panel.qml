...
Panel {
    ...
    function applyTheme(themeName) {
        ...
        var script = 'THEME="$0"; '
            ...
            + 'mkdir -p "$DEST/apps"; '
            + 'PARENT="${THEME%%-*}"; '
            + 'if [ "$PARENT" != "$THEME" ]; then '
            + '  for b in "$HOME/.local/share/icons" "/usr/share/icons" "$HOME/.icons"; do '
            + '    for sub in "256x256/apps" "128x128/apps" "64x64/apps" "48x48/apps" "32x32/apps" "scalable/apps" "apps"; do '
            + '      if [ -d "$b/$PARENT/$sub" ]; then '
            + '        cp -asf "$b/$PARENT/$sub"/* "$DEST/apps/" 2>/dev/null || true; '
            + '      fi; '
            + '    done; '
            + '  done; '
            + 'fi; '
            + 'for b in "$HOME/.local/share/icons" "/usr/share/icons" "$HOME/.icons"; do '
            + '  for sub in "256x256/apps" "128x128/apps" "64x64/apps" "48x48/apps" "32x32/apps" "scalable/apps" "apps"; do '
            + '    if [ -d "$b/$THEME/$sub" ]; then '
            + '      cp -asf "$b/$THEME/$sub"/* "$DEST/apps/" 2>/dev/null || true; '
            + '    fi; '
            + '  done; '
            + 'done'
