...
Panel {
    ...
    function applyTheme(themeName) {
        ...
        var script = 'THEME="$0"; '
            ...
            + '  if [ -d "$pdir" ]; then find "$pdir" -maxdepth 1 \\( -name "*.svg" -o -name "*.png" \\) ! -name "*symbolic*" -exec ln -sf -t "$DEST/apps/" {} + 2>/dev/null || true; fi; '
            + 'done; '
            ...
            + '      if [ -d "$hbase/$s/apps" ]; then find "$hbase/$s/apps" \\( -name "*.svg" -o -name "*.png" \\) ! -name "*symbolic*" -exec ln -sf -t "$DEST/apps/" {} + 2>/dev/null || true; fi; '
            + '    done; '
            + '  fi; '
            + 'done; '
            ...
            + '        if [ -d "$pb/$p" ]; then find "$pb/$p" \\( -path "*/apps/*" -o -path "*/applications/*" \\) \\( -name "*.svg" -o -name "*.png" \\) ! -name "*symbolic*" -exec ln -sf -t "$DEST/apps/" {} + 2>/dev/null || true; fi; '
            + '      done; '
            + '    done; '
            + '    break; '
            + '  fi; '
            + 'done; '
            + 'for b in "$HOME/.local/share/icons" "/usr/share/icons" "$HOME/.icons"; do '
            + '  if [ -d "$b/$THEME" ]; then find "$b/$THEME" \\( -path "*/apps/*" -o -path "*/applications/*" \\) \\( -name "*.svg" -o -name "*.png" \\) ! -name "*symbolic*" -exec ln -sf -t "$DEST/apps/" {} + 2>/dev/null || true; fi; '
            + 'done; '
            + 'make_alias() { '
            + '  local r="$1"; local a="$2"; '
            + '  for ext in svg png; do '
            + '    if [ -e "$DEST/apps/$r.$ext" ] && [ ! -e "$DEST/apps/$a.$ext" ]; then '
            + '      ln -sf "$DEST/apps/$r.$ext" "$DEST/apps/$a.$ext" 2>/dev/null || true; '
            + '    fi; '
            + '  done; '
            + '}; '
            + 'for f in "$DEST/apps/"*.*; do '
            + '  [ -e "$f" ] || continue; '
