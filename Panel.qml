...
Panel {
    ...
    function applyTheme(themeName) {
        ...
        var script = 'THEME="$0"; '
            ...
            + '  fi; '
            + 'done; '
            + 'DATA_DIR="$HOME/.local/share/icons/0-active-theme-data"; '
            ...
            + '  if [ -d "$pdir" ]; then find "$pdir" -maxdepth 1 \\( -name "*.svg" -o -name "*.png" \\) ! -name "*symbolic*" -exec ln -sf -t "$DATA_DIR/apps/" {} + 2>/dev/null || true; fi; '
            + 'done; '
            + 'for hbase in /usr/share/icons/hicolor "$HOME/.local/share/icons/hicolor"; do '
            + '  if [ -d "$hbase" ]; then '
            + '    for s in 48x48 64x64 128x128 256x256 512x512 scalable; do '
            + '      if [ -d "$hbase/$s/apps" ]; then find "$hbase/$s/apps" \\( -name "*.svg" -o -name "*.png" \\) ! -name "*symbolic*" -exec ln -sf -t "$DATA_DIR/apps/" {} + 2>/dev/null || true; fi; '
            + '    done; '
            + '  fi; '
            + 'done; '
            + 'for b in "$HOME/.local/share/icons" "/usr/share/icons" "$HOME/.icons"; do '
            + '  if [ -f "$b/$THEME/index.theme" ]; then '
            + '    parents=$(grep -E "^Inherits=" "$b/$THEME/index.theme" | cut -d= -f2 | tr "," " "); '
            + '    for p in $parents; do '
            + '      [ "$p" = "hicolor" ] && continue; '
            + '      for pb in "$HOME/.local/share/icons" "/usr/share/icons" "$HOME/.icons"; do '
            + '        if [ -d "$pb/$p" ]; then find "$pb/$p" \\( -path "*/apps/*" -o -path "*/applications/*" \\) \\( -name "*.svg" -o -name "*.png" \\) ! -name "*symbolic*" -exec ln -sf -t "$DATA_DIR/apps/" {} + 2>/dev/null || true; fi; '
            + '      done; '
            + '    done; '
            + '    break; '
            + '  fi; '
            + 'done; '
            + 'for b in "$HOME/.local/share/icons" "/usr/share/icons" "$HOME/.icons"; do '
            + '  if [ -d "$b/$THEME" ]; then find "$b/$THEME" \\( -path "*/apps/*" -o -path "*/applications/*" \\) \\( -name "*.svg" -o -name "*.png" \\) ! -name "*symbolic*" -exec ln -sf -t "$DATA_DIR/apps/" {} + 2>/dev/null || true; fi; '
            + 'done; '
            + 'make_alias() { '
            + '  local r="$1"; local a="$2"; '
            + '  for ext in svg png; do '
            + '    if [ -e "$DATA_DIR/apps/$r.$ext" ] && [ ! -e "$DATA_DIR/apps/$a.$ext" ]; then '
            + '      ln -sf "$DATA_DIR/apps/$r.$ext" "$DATA_DIR/apps/$a.$ext" 2>/dev/null || true; '
            + '    fi; '
            + '  done; '
            + '}; '
            + 'for f in "$DATA_DIR/apps/"*.*; do '
            + '  [ -e "$f" ] || continue; '
            + '  base=$(basename "$f"); ext="${base##*.}"; name="${base%.*}"; '
            + '  if [[ "$name" == *.* ]]; then '
            + '    last="${name##*.}"; '
            + '    if [ "$last" = "desktop" ] || [ "$last" = "client" ] || [ "$last" = "app" ]; then '
            + '      without_last="${name%.*}"; mid="${without_last##*.}"; [ -n "$mid" ] && make_alias "$name" "$mid"; '
            + '    elif [ -n "$last" ] && [ "$last" != "$name" ]; then make_alias "$name" "$last"; fi; '
            + '    no_prefix=$(echo "$name" | sed -E "s/^(com|org|net|io|dev|app|ru)\\.//I"); '
            + '    [ -n "$no_prefix" ] && [ "$no_prefix" != "$name" ] && make_alias "$name" "$no_prefix"; '
            + '  fi; '
            + 'done; '
