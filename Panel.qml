...
Panel {
    ...
    function applyTheme(themeName) {
        ...
        var script = 'THEME="$0"; '
            ...
            + '  if [ -d "$hbase" ]; then '
            + '    for d in $(find "$hbase" -type d -name "apps" 2>/dev/null); do '
            ...
            + '    if [ -d "$pb/$p" ]; then '
            + '      for d in $(find "$pb/$p" -type d \\( -name "apps" -o -name "applications" \\) 2>/dev/null); do '
            + '        cp -asf "$d"/* "$NEW_DIR/apps/" 2>/dev/null || true; '
            + '      done; '
            + '    fi; '
            + '  done; '
            ...
            + 'for b in "$HOME/.local/share/icons" "/usr/share/icons" "$HOME/.icons"; do '
            + '  if [ -d "$b/$THEME" ]; then '
            + '    for d in $(find "$b/$THEME" -type d \\( -name "apps" -o -name "applications" \\) 2>/dev/null); do '
            + '      cp -asf "$d"/* "$NEW_DIR/apps/" 2>/dev/null || true; '
            + '    done; '
            + '  fi; '
            + 'done; '
            ...
            + 'for png in "$NEW_DIR/apps/"*.png; do '
            + '  [ -f "$png" ] || continue; '
            + '  base_name="${png%.png}"; '
            + '  svg_target="${base_name}.svg"; '
            + '  if [ ! -f "$svg_target" ]; then '
            + '    real_png=$(realpath -e "$png" 2>/dev/null || true); '
            + '    if [ -n "$real_png" ]; then '
            + '      printf \'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><image href="file://%s" width="512" height="512"/></svg>\\n\' "$real_png" > "$svg_target"; '
            + '    fi; '
            + '  fi; '
            + 'done; '
            + 'rm -rf "$LINK.old" 2>/dev/null || true; '
            + '[ -e "$LINK" ] && mv "$LINK" "$LINK.old" 2>/dev/null || true; '
            + 'mv -Tf "$NEW_DIR" "$LINK"; '
            + 'rm -rf "$LINK.old" 2>/dev/null || true; '
            + 'cleanup_stale; '
