...
Panel {
    ...
    function applyTheme(themeName) {
        ...
        var script = 'THEME="$0"; '
            ...
            + '  [ -f "$DEST/apps/org.telegram.desktop.$ext" ] && ln -sf "$DEST/apps/org.telegram.desktop.$ext" "$DEST/apps/telegram.$ext" && ln -sf "$DEST/apps/org.telegram.desktop.$ext" "$DEST/apps/org.telegram.$ext" 2>/dev/null || true; '
            + '  [ -f "$DEST/apps/google-chrome.$ext" ] && ln -sf "$DEST/apps/google-chrome.$ext" "$DEST/apps/chrome.$ext" 2>/dev/null || true; '
            + '  [ -f "$DEST/apps/vscode.$ext" ] && ln -sf "$DEST/apps/vscode.$ext" "$DEST/apps/code.$ext" 2>/dev/null || true; '
            + '  [ -f "$DEST/apps/code.$ext" ] && ln -sf "$DEST/apps/code.$ext" "$DEST/apps/vscode.$ext" 2>/dev/null || true; '
            + 'done'
