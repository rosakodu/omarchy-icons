...
Panel {
    ...
    function applyTheme(themeName) {
        ...
        var script = 'THEME="$0"; '
            ...
            + '    if [ -d "$pb/$p" ]; then '
            + '      find "$pb/$p" -type f \( -name "*.svg" -o -name "*.png" \) \( -path "*/apps/*" -o -path "*/applications/*" \) -exec cp -asf -t "$NEW_DIR/apps/" {} + 2>/dev/null || true; '
            + '    fi; '
            + '  done; '
            + 'done; '
            + 'for b in "$HOME/.local/share/icons" "/usr/share/icons" "$HOME/.icons"; do '
            + '  if [ -d "$b/$THEME" ]; then '
            + '    find "$b/$THEME" -type f \( -name "*.svg" -o -name "*.png" \) \( -path "*/apps/*" -o -path "*/applications/*" \) -exec cp -asf -t "$NEW_DIR/apps/" {} + 2>/dev/null || true; '
            + '  fi; '
            + 'done; '
