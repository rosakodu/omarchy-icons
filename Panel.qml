...
Panel {
    ...
    function applyTheme(themeName) {
        ...
        var script = 'THEME="$0"; '
            ...
            + '  if [ -d "$hbase" ]; then '
            + '    find "$hbase" -type f \( -name "*.svg" -o -name "*.png" \) \( -path "*/apps/*" -o -path "*/applications/*" \) -exec cp -asf -t "$NEW_DIR/apps/" {} + 2>/dev/null || true; '
            + '  fi; '
            + 'done; '
            + 'resolve_parents() { '
            + '  local t="$1"; '
            ...
            + '  local src="$1"; '
            + '  [ ! -d "$src" ] && return; '
            + '  for sz in 16 22 24 32 48 64 128 256 512 48x48 64x64 128x128 256x256 512x512; do '
            + '    find "$src" -type f \( -name "*.svg" -o -name "*.png" \) \( -path "*/apps/$sz/*" -o -path "*/applications/$sz/*" \) -exec cp -asf -t "$NEW_DIR/apps/" {} + 2>/dev/null || true; '
            + '  done; '
            + '  find "$src" -type f \( -name "*.svg" -o -name "*.png" \) \( -path "*/apps/scalable/*" -o -path "*/applications/scalable/*" -o -path "*/apps/*" -o -path "*/applications/*" \) -exec cp -asf -t "$NEW_DIR/apps/" {} + 2>/dev/null || true; '
            + '}; '
            ...
            + '    link_leaf_icons "$pb/$p"; '
            + '  done; '
            + 'done; '
            + 'for b in "$HOME/.local/share/icons" "/usr/share/icons" "$HOME/.icons"; do '
            + '  link_leaf_icons "$b/$THEME"; '
            + 'done; '
