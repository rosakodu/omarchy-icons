...
Panel {
    ...
    function applyTheme(themeName) {
        ...
        var script = 'THEME="$0"; '
            ...
            + '  tgt=""; '
            + '  for c in org.telegram.desktop telegram-desktop telegram; do '
            + '    if [ -f "$DEST/apps/$c.$ext" ] && [ ! -L "$DEST/apps/$c.$ext" -o -e "$DEST/apps/$c.$ext" ]; then tgt="$DEST/apps/$c.$ext"; break; fi; '
            + '  done; '
            + '  if [ -n "$tgt" ]; then ln -sf "$tgt" "$DEST/apps/telegram.$ext" 2>/dev/null; ln -sf "$tgt" "$DEST/apps/org.telegram.$ext" 2>/dev/null; ln -sf "$tgt" "$DEST/apps/org.telegram.desktop.$ext" 2>/dev/null; fi; '
            + '  tgt=""; '
            + '  for c in com.visualstudio.code visual-studio-code visualstudiocode code-oss vscode code; do '
            + '    if [ -f "$DEST/apps/$c.$ext" ] && [ ! -L "$DEST/apps/$c.$ext" -o -e "$DEST/apps/$c.$ext" ]; then tgt="$DEST/apps/$c.$ext"; break; fi; '
            + '  done; '
            + '  if [ -n "$tgt" ]; then ln -sf "$tgt" "$DEST/apps/code.$ext" 2>/dev/null; ln -sf "$tgt" "$DEST/apps/vscode.$ext" 2>/dev/null; ln -sf "$tgt" "$DEST/apps/com.visualstudio.code.$ext" 2>/dev/null; fi; '
            + '  tgt=""; '
            + '  for c in google-chrome google-chrome-stable chromium chrome; do '
            + '    if [ -f "$DEST/apps/$c.$ext" ] && [ ! -L "$DEST/apps/$c.$ext" -o -e "$DEST/apps/$c.$ext" ]; then tgt="$DEST/apps/$c.$ext"; break; fi; '
            + '  done; '
            + '  if [ -n "$tgt" ]; then ln -sf "$tgt" "$DEST/apps/chrome.$ext" 2>/dev/null; ln -sf "$tgt" "$DEST/apps/google-chrome.$ext" 2>/dev/null; fi; '
            + 'done'
